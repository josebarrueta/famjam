...
grep -q '^8|008_calendar_sources.sql|test-release|64$' <<<"$ledger"

run_migrations pre >/dev/null
[[ $(psql "$DATABASE_URL" -Atc 'SELECT count(*) FROM schema_migrations') == "8" ]]
...
SQL
run_migrations pre >/dev/null
[[ $(psql "$DATABASE_URL" -Atc "SELECT count(*) FROM schema_migrations WHERE version IS NOT NULL AND checksum IS NOT NULL AND app_version = 'legacy-unrecorded'") == "8" ]]
...
cat >"$tmp/failing-migrations/pre/009_atomic_marker.sql" <<'SQL'
CREATE TABLE migration_atomic_marker (id integer PRIMARY KEY);
SQL
cat >"$tmp/failing-migrations/pre/010_intentional_failure.sql" <<'SQL'
...
[[ $(psql "$DATABASE_URL" -Atc "SELECT count(*) FROM schema_migrations WHERE version IN (9, 10)") == "0" ]]

cat >"$tmp/failing-migrations/post/009_post_release_marker.sql" <<'SQL'
CREATE TABLE post_release_marker (id integer PRIMARY KEY);
SQL
rm "$tmp/failing-migrations/pre/009_atomic_marker.sql" "$tmp/failing-migrations/pre/010_intentional_failure.sql"
...
[[ $(psql "$DATABASE_URL" -Atc "SELECT app_version FROM schema_migrations WHERE version = 9") == "test-post-release" ]]

cat >"$tmp/failing-migrations/pre/009_duplicate_version.sql" <<'SQL'
SELECT 1;
SQL
...
grep -q 'Duplicate migration version 9' "$tmp/duplicate.out"
